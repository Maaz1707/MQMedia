...
export default function HeroRing({ scrollYProgress }: HeroRingProps) {
  ...
  const exitScale = useTransform(scrollYProgress, [0, 1], [1, 0.82]);

  // Scroll-velocity response: the ring stretches and tilts with how fast
  // you're scrolling, then eases back to rest — a physical, inertial feel
  // rather than a fixed mechanical spin.
  const { scrollY } = useScroll();
  const rawVelocity = useVelocity(scrollY);
  const smoothVelocity = useSpring(rawVelocity, { stiffness: 260, damping: 32 });
  const velocityScale = useTransform(smoothVelocity, [-2500, 0, 2500], [1.1, 1, 1.1]);
  const velocitySkew = useTransform(smoothVelocity, [-2500, 0, 2500], [-7, 0, 7]);

  return (
    <motion.div
      aria-hidden="true"
      style={{ opacity, scale: exitScale }}
      className="pointer-events-none absolute inset-0 flex items-center justify-center"
    >
      <motion.div style={{ scale: velocityScale, skewY: velocitySkew }}>
        <motion.div
          animate={{ rotate: 360 }}
